...
class plxMedias {
	...
	public function moveDirs($dirs, $src_dir, $dst_dir) {
		...
		if(sizeof($dirs)==1) {
			if($count==0)
				return plxMsg::Error(L_PLXMEDIAS_MOVE_FOLDER_ERR);
			else
				return plxMsg::Info(L_PLXMEDIAS_MOVE_FOLDER_SUCCESSFUL);
		}
		else {
			if($count==0)
				return plxMsg::Error(L_PLXMEDIAS_MOVE_FOLDERS_ERR);
			else
				return plxMsg::Info(L_PLXMEDIAS_MOVE_FOLDERS_SUCCESSFUL);
		}
